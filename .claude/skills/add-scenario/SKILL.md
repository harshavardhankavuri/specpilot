---
name: add-scenario
description: Scaffold a new SpecPilot scenario or case file (*.scenario.yaml / *.cases.yaml) against a synced OpenAPI operationId, without re-deriving the field format from source each time.
---

# Add a SpecPilot scenario

Field reference lives in `docs/scenario-format.md` — read that instead of
`src/schema/scenarioSchema.ts`. This skill just gives the fast path.
...
4. **Use `operation:` for anything the spec can express directly.** Only reach for `custom:` (see
   `/add-custom-step`) for setup, transforms, or multi-call logic the spec can't express.
...
- Don't re-read `docs/scenario-format.md` in full if you already have it in this session's context
  from an earlier scenario — only re-check the specific field you're unsure about.
