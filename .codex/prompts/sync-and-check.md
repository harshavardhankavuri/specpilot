...
3. Triage failures from the command output alone — don't open `.spec-cache/spec.json` or re-read
   ...
   - Missing/extra required param → open only the one flagged scenario file, not the whole
     `scenarios/` directory.
4. Remember `check` does **not** verify `custom:`/`cleanup:` export names (see the
   `add-custom-step` prompt) — a clean `check` doesn't guarantee those resolve; only
   `apitest run` does.
...
- Never open the full `.spec-cache/` cache file to "double check" a `check` failure the CLI
