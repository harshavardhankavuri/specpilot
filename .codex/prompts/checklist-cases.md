...
1. **Get the operation.** Ensure `.spec-cache/` is fresh (`npx apitest sync` if stale/missing).
   ...
   only if you need a raw field `describe` doesn't surface. A category with no matching documented
   status/constraint is out of scope — don't invent one.
...
5. **Reuse existing custom steps before writing new ones.** Grep `customSteps/*.ts` for a step
   ...
   export names across `customSteps/*.ts` collide silently.

6. **Write the `cases:` block** into the target scenario file — append to an existing
   `.scenario.yaml`/`.cases.yaml` for this resource if one exists, otherwise create
   ...
   why — this stops a future edit from re-adding a case for something the API genuinely doesn't
   support.
...
- **`assert.jsonpath` does not interpolate `{{var}}`.** Only `params`/`query`/`headers`/`body`
  and assertion `equals`/`contains`/`oneOf`/`db` `query`/`params` do. A generated case that needs
  a dynamic value *inside* the jsonpath expression itself needs a `custom:` step doing the
  comparison in TypeScript instead of a declarative filter path — see
  `assertBookingListIsOwnBookingsOnly` in `customSteps/bookingEdgeCases.ts` for the pattern.
...
- Pull only the one operation's descriptor via `apitest describe` — never dump the whole
  `.spec-cache/spec.json` into context.
- Don't re-derive `docs/scenario-format.md`/`docs/custom-steps.md` field syntax from source if you
  already have it from earlier in this session.
